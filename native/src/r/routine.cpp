...
namespace datasuite
{
    namespace routines
    {
        ...
        wrapped_return<std::string> sexpToString(SEXP s)
        {
            ...
            if (Rf_length(s) < 1)
                return {
                    std::string(), std::nullopt}; // return empty string for empty character vector
            ...
            SEXP headers)
        {
            ...
            for (const auto &kv : headers_map)
                js_headers.set(kv.first, kv.second);

            // get the interpreter signleton as this is holding the js-function
            // we need to call
            auto &wasm_interpreter = static_cast<wasm_interpreter &>(get_interpreter());
...
#ifdef EMSCRIPTEN
            {"datasuite_download_file", (DL_FUNC)&routines::datasuite_download_file, 8},
#endif
