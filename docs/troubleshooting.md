...
|---|---|
| `jovian: supervisor executable not found at … Run the native build (npm run build:native) before creating a session.` | `themisto` is not built where the library looks (`dist/native/Release` next to `dist/lib`, or `$JOVIAN_NATIVE_DIR`). |
