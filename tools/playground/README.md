...
- **Shortcuts:** `Shift+Enter` run, `Tab` complete, `Shift+Tab` / `Ctrl+I` inspect, `Ctrl+L` clear output, `Esc` interrupt a running cell (it closes an open completion list first). An interrupted cell ends with "Execution interrupted".
...
- **Completion and inspect**, both real Jupyter requests:
  - `Tab` in the input completes the token at the caret (common prefix first, then a picker: ↑/↓, `Enter`/`Tab` accept, `Esc` closes). At the start of a line `Tab` indents.
